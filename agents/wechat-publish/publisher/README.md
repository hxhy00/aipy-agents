...
排版能力来自独立 skill 包 **`wechat-article-sop-layout.zip`**（需单独安装到 AiPy）：

| 交付物 | 形式 | 作用 |
|--------|------|------|
| `wechat-sop-publish-assistant.mcpb` | 智能体包 | 工具层：解析终稿、写草稿箱（含兼容性硬校验）、文字校验、兼容性自检 |
| `wechat-article-sop-layout.zip` | skill 包 | 知识层：排版规范、风格矩阵、视觉素材库、三个预检脚本 + 无损压缩脚本 |
